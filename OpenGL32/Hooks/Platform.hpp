...
class Any
{
    private:
        ...
        template<typename T>
        struct Info: public Storage
        {
            T Value;
            Info(T Value) : Value(Value) {}
        };
    ...
    public:
        template<typename T>
        Any(T Value);
        Any(const Any &Value) = delete;
        Any(Any &&Value);
        ~Any() {delete Data;}

        Any& operator = (const Any &Value) = delete;
...
template<typename T>
Any::Any(T Value) : Data(new Info<T>(Value)) {}

template<typename T>
T Any::Value()
{
    if (Info<T>* Ptr = dynamic_cast<Info<T>*>(this->Data))
    {
        return Ptr->Value;
    }
    return nullptr;
